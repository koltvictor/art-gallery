...
CSV.foreach("lib/data.csv") do |row|
  Art.create(user_id: 1, title: row[4], artist: row[1], creation_date: row[5], number_of: row[6], medium: row[9], image_height: row[13], image_width: row[14], frame_height: row[16], frame_width: row[17], image: row[41])
end
